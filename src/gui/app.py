...
class App(ctk.CTk):
    ...
    def _build_sidebar(self) -> None:
        ...
        # NOTE:
        # On environments using Tk 9.x, CustomTkinter's CTkOptionMenu can fail
        # during initialization (TclError when clearing menu indexes). We keep
        # the app runnable by gracefully degrading this control.
        try:
            ...
            logger.warning(
                "No se pudo crear el selector de tema (fallback activo): %s", exc
            )
            fallback_label = ctk.CTkLabel(
                self._sidebar,
                text="Tema: dark (auto)",
            )
            fallback_label.grid(row=12, column=0, padx=20, pady=(5, 20))
