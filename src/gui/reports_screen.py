...
class ReportsScreen(ctk.CTkFrame):
    ...
    def __init__(self, master, db_manager: "DatabaseManager") -> None:
        ...
        self._offset = 0
        self._selected_person = "Todas las personas"
    ...
    def _render_stats(self) -> None:
        ...
        try:
            ...
            ctk.CTkLabel(self._stats_frame, text="Error al cargar estadísticas.").pack(
                expand=True
            )
            return
    ...
    def _build_person_chart_tab(self, parent) -> None:
        ...
        ctk.CTkLabel(filter_frame, text="Filtrar por persona:").pack(
            side="left", padx=(10, 5)
        )

        # NOTE:
        # In Tk 9.x environments, CustomTkinter dropdown-based widgets
        # (CTkComboBox/CTkOptionMenu) can raise TclError on init.
        # Fallback to a static label + "todas" mode to keep the app usable.
        try:
            ...
            logger.warning(
                "No se pudo crear filtro por persona (fallback activo): %s", exc
            )
            self._person_filter = None
            ctk.CTkLabel(
                filter_frame,
                text="Todas las personas (modo compatibilidad)",
    ...
    def _render_person_chart(self) -> None:
        ...
        selection = self._selected_person
        if self._person_filter is not None:
    ...
    def _show_export_message(self, message: str) -> None:
        ...
        ctk.CTkButton(dialog, text="Aceptar", command=dialog.destroy, width=100).pack(
            pady=(0, 15)
        )
    ...
    def _refresh_table(self) -> None:
        ...
        logs = self.db_manager.get_detection_logs(limit=_PAGE_SIZE, offset=self._offset)
