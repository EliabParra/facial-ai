...
class RegistrationScreen(ctk.CTkFrame):
    ...
    _PREVIEW_W = 480
    _PREVIEW_H = 360
    _CAMERA_DEVICE_CANDIDATES = ("/dev/video0", "/dev/video1")
    _CAMERA_INDEX_CANDIDATES = (0, 1, 2)
    ...
    def _build_ui(self) -> None:
        ...
        ctk.CTkLabel(left, text="Calidad del rostro:", font=ctk.CTkFont(size=12)).grid(
            row=12, column=0, padx=15, pady=(15, 2), sticky="w"
        )
        ...
        self._persons_list_frame = ctk.CTkScrollableFrame(left, width=200, height=120)
        self._persons_list_frame.grid(
            row=17, column=0, padx=15, pady=(0, 5), sticky="ew"
        )
        ...
        # Mensaje de estado
        self._status_label = ctk.CTkLabel(left, text="Estado: Listo", wraplength=220)
    ...
    def _start_camera(self) -> None:
        ...
        if self._cap is None:
            self._set_status(
                "❌ Error: No se pudo abrir la cámara (índices 0/1/2).",
                error=True,
            )
            return
    ...
    def _stream_loop(self) -> None:
        ...
        try:
            while self._streaming and self._cap and self._cap.isOpened():
                ...
                self.after(0, self._update_preview, annotated)
        finally:
            self.after(0, self._on_stream_stopped)

    def _on_stream_stopped(self) -> None:
        """Normalizar estado UI/recursos al detener stream."""
        self._streaming = False
        if self._cap:
            self._cap.release()
            self._cap = None
    ...
    def _capture_frame(self) -> None:
        ...
        if not faces:
            self._set_status(
                "❌ No se detectó ningún rostro. Intenta de nuevo.", error=True
            )
            return
        ...
        self._set_status(
            f"✅ Captura {count}/{_MAX_CAPTURES} guardada (calidad: {quality['score']}%)"
        )
    ...
    def _reset_captures(self) -> None:
        ...
        self._capture_btn.configure(
            text="📸 Capturar (0/5)", state="normal" if self._streaming else "disabled"
        )
        self._register_btn.configure(state="disabled")
        self._quality_bar.set(0)
    ...
    def _update_quality_indicator(self, quality: dict) -> None:
        ...
        self._quality_label.configure(text=f"{indicator} ({score}%)", text_color=color)
    ...
    def _open_camera(self) -> Optional[cv2.VideoCapture]:
        """Intentar abrir cámara por device path y luego por índice."""
        # 1) Camino más estable en Linux: abrir nodo explícito /dev/videoX
        for device in self._CAMERA_DEVICE_CANDIDATES:
        ...
        # 2) Fallback clásico por índice (depende de backend)
        for idx in self._CAMERA_INDEX_CANDIDATES:
    ...
    def _refresh_persons_list(self) -> None:
        ...
        for person in people:
            ...
            row_frame = ctk.CTkFrame(self._persons_list_frame, fg_color="transparent")
            ...
            ctk.CTkButton(
                ...
                hover_color="#962d22",
                font=ctk.CTkFont(size=12),
                command=lambda pid=person["id"], pn=full_name: self._delete_person(
                    pid, pn
                ),
    ...
    def _delete_person(self, person_id: int, person_name: str) -> None:
        ...
        def _confirm():
            ...
            if success:
                ...
                self._set_status(
                    f"❌ No se pudo eliminar a '{person_name}'.", error=True
                )
